...
import static com.android.doral.directions.MathUtil.*;
import static com.android.doral.directions.MathUtil.EARTH_RADIUS;
import static com.android.doral.directions.MathUtil.clamp;
import static com.android.doral.directions.MathUtil.hav;
import static com.android.doral.directions.MathUtil.havDistance;
import static com.android.doral.directions.MathUtil.havFromSin;
import static com.android.doral.directions.MathUtil.inverseMercator;
import static com.android.doral.directions.MathUtil.mercator;
import static com.android.doral.directions.MathUtil.sinFromHav;
import static com.android.doral.directions.MathUtil.sinSumFromHav;
import static com.android.doral.directions.MathUtil.wrap;
import static com.android.doral.directions.SphericalUtil.computeDistanceBetween;
...
public class PolyUtil {
     ...
     * @return
     */
    public static LatLng getNearestLocationOnEdgeOrPath(LatLng point, List<LatLng> poly, boolean closed, boolean geodesic, double toleranceEarth) {
        int size = poly.size();
        if (size == 0) {
            return point;
        }
    ...
    public static LatLng getNearestLocationOnEdgeOrPath(LatLng inputLatLng, List<LatLng> poly, double minimumDistanceInMeter) {
        float distance = 0;
        boolean firstTimeOnly = true;

        LatLng nearestLatLng = inputLatLng;

        if (poly != null && !poly.isEmpty()) {
            for (int p = 0; p < poly.size(); p++) {
                Location polyPoint = new Location("polyPoint");
                polyPoint.setLatitude(poly.get(p).latitude);
                polyPoint.setLongitude(poly.get(p).longitude);

                Location findPoint = new Location("findPoint");
                findPoint.setLatitude(inputLatLng.latitude);
                findPoint.setLongitude(inputLatLng.longitude);

                float tempDistance = polyPoint.distanceTo(findPoint);

                if (tempDistance < minimumDistanceInMeter) {
                    if (firstTimeOnly) {
                        firstTimeOnly = false;
                        distance = tempDistance;
                        nearestLatLng = poly.get(p);
                    }
